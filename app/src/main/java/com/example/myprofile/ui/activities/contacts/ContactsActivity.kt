...
class ContactsActivity : AppCompatActivity(), AddContactCallback, OnContactClickListener {
    ...
    private val viewModel: ContactsViewModel by viewModels {
        ContactsViewModelFactory(       //todo hilt
            ContactsRepository()
        )
    }
    private lateinit var adapter: ContactsAdapter   //todo by lazy
    ...
    private fun showUndoSnackbar() {
        Snackbar.make(binding.root, R.string.contact_deleted, Snackbar.LENGTH_SHORT)
            .setAction(R.string.undo) {
                viewModel.restoreLastDeletedContact() //todo viewmodel must hold last deleted contact and his position
            }
            .show()
    }
    ...
    private fun showDialog(isLargeLayout: Boolean) {
        ...
        if (!isLargeLayout) {
            ...
            newFragment.show(supportFragmentManager, ADD_CONTACT_DIALOG)
        } else {
